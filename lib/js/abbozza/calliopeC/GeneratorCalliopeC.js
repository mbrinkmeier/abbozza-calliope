...
AbbozzaGenerator.prototype.workspaceToCode = function (opt_workspace) {
    ...
    for (var x = blocks.length - 1; x >= 0; x--) {
        ...
        if (block) {  // catching missing device block
            ...
            if (line) {
                if ( (block.type == "func_decl") || (block.type == "int_ist") ) {
    ...
    code = code.replace(/###operations###/g, "\t" + blockcode);
    ...
    if (this.radioRequired == true) {
        this.addSetupCode("#undef MICROBIT_BLE_ENABLE\n");
        this.addSetupCode("#define MICROBIT_BLE_ENABLE 0\n");
        this.addInitCode("abbozza.radio.enable();");
    }
  ...
  if ( this.parserRequired == true ) {
    ...
    // Add the inithook for other blocks
    if ( this.initHookCode != "" ) {
       this.initHookCode = "// init hook\n" + this.initHookCode;
